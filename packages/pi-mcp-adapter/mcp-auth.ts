...
function writeSecureAuthEntry(serverName: string, entry: AuthEntry): void {
  ...
  try {
    ...
    throw new Error(`Failed to write OAuth credentials for ${serverName} to the OS secure credential store`, {
      cause: error,
    });
  }
}
...
function readAuthEntry(serverName: string, options?: AuthStorageOptions): AuthEntry | undefined {
  ...
  try {
    ...
    throw new Error(`Failed to read OAuth credentials for ${serverName} from the OS secure credential store`, {
      cause: error,
    });
  }
...
export function removeAuthEntry(serverName: string, options?: AuthStorageOptions): void {
  ...
  try {
    ...
    throw new Error(`Failed to remove OAuth credentials for ${serverName} from the OS secure credential store`, {
      cause: error,
    });
  }
  removeLegacyAuthEntry(serverName, options);
