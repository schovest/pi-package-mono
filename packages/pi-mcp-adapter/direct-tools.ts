...
import { type UiSessionRuntime, maybeStartUiSession, summarizeUiSessionResult } from "./ui-session.ts";
...
export function buildProxyDescription(
  ...
  for (const serverName of Object.keys(config.mcpServers)) {
    ...
    const toolCount = (entry?.tools ?? []).filter((tool) =>
      isToolAllowed(tool.name, serverName, prefix, definition.includeTools, definition.excludeTools),
    ).length;
    const resourceCount =
