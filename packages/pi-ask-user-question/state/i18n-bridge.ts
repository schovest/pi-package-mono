 ...
 * call site routes through one place. Backed by `@schovest/pi-i18n`'s SDK
 * when available; degrades to canonical-English fallbacks when not.
 *
 * - `t(key, fallback)` is `scope("@schovest/pi-ask-user-question")` if the
...
export const I18N_NAMESPACE = "@schovest/pi-ask-user-question";
...
try {
  const sdk = (await import("@schovest/pi-i18n")) as I18nSDK;
