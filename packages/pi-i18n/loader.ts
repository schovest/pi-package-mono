 ...
 *
 * Lives under the `@schovest/pi-i18n/loader` subpath (separate from the
 * SDK entry) so a consumer doing `await import("@schovest/pi-i18n/loader")`
 * does not pull `i18n-ui.ts` and the pi-tui dependency into its load graph
 * just to register strings.
 *
 * Soft-peer contract preserved: when `@schovest/pi-i18n` is not installed
...
import { SUPPORTED_LOCALES, type TranslationMap, registerStrings } from "./i18n.js";
...
function loadOneLocale(packageUrl: string, code: string, label: string): TranslationMap {
  try {
    ...
    console.warn(`${label}: failed to load locales/${code}.json — falling back to English (${(err as Error).message})`);
    return {};
  }
}
