...
function checkGoalCompletion(state: GoalState): CompletionResult {
  ...
  const allTerminal = state.tasks.every((t) => t.status === "done" || t.status === "failed" || t.status === "skipped");
...
export default function goalExtension(pi: ExtensionAPI): void {
  ...
  const updateGoalSchema = Type.Object({
    action: Type.Union([Type.Literal("set_tasks"), Type.Literal("update_task"), Type.Literal("complete")]),
    tasks: Type.Optional(
      Type.Array(
        Type.Object({
  ...
  pi.on("agent_end", (_event, ctx) => {
    ...
    switch (completion) {
      ...
      case "failed": {
        ...
        persistGoal(pi);
        const reason =
          activeGoal.turnCount >= activeGoal.maxTurns ? `达到最大轮次 (${activeGoal.maxTurns})` : "有子任务失败";
