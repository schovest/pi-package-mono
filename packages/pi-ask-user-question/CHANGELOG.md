...
All notable changes to `@schovest/pi-ask-user-question` are documented here.
...
- Emit `rpiv:ask-user:aborted` (`{ aborted: true }`) when the user aborts the questionnaire
  by pressing Esc (TUI) or dismissing a dialog (RPC walker), so external listeners can
  react to the abort. The questionnaire still closes with `cancelled: true` exactly as
  before; the event is never emitted for validation errors or hosts that never showed
  the questions.
...
- `rpiv:ask-user:prompt` event emitted before showing the questionnaire, with a typed payload and importable event contract from `@schovest/pi-ask-user-question/events`.
...
- `@schovest/pi-i18n` is now a soft optional peer (`peerDependenciesMeta.optional: true`). The bridge and module-init `registerStrings` call use a dynamic `await import()` inside try/catch, so a standalone install of just `rpiv-ask-user-question` no longer fails with `Cannot find module '@schovest/pi-i18n'` — the dialog stays online with English-only UI when the SDK isn't present, and lights up localization automatically when it is.
...
- Localized TUI strings via `@schovest/pi-i18n`: sentinel rows ("Type something.", "Chat about this", "Next"), submit/cancel labels, dialog hints, review-tab heading and prompts, preview placeholder, notes affordance, and the chat-summary line all resolve at render time through the i18n bridge.
