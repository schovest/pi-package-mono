...
class CollapsibleText implements Component {
  ...
  render(width: number): string[] {
    if (this.cachedLines !== undefined && this.cachedText === this.text && this.cachedWidth === width) {
      return this.cachedLines;
    }

    const lines = new Text(this.text, 0, 0).render(width);
    let result: string[];
    if (this.expanded || lines.length <= this.maxCollapsedLines) {
      result = lines;
    } else {
      result = [
...
export function renderMcpToolResult(
  ...
  if (output.length > MAX_RESULT_OUTPUT_CHARS) {
    const remaining = output.length - MAX_RESULT_OUTPUT_CHARS;
    const label = remaining === 1 ? "char" : "chars";
    const truncatedOutput =
      output.slice(0, MAX_RESULT_OUTPUT_CHARS) +
      `\n${activeTheme.fg("muted", `... (${remaining} more ${label} hidden)`)}`;
    return new CollapsibleText(
      truncatedOutput,
      expanded,
      DEFAULT_MAX_COLLAPSED_LINES + (identity ? 1 : 0) + 1,
