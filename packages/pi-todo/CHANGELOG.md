...
All notable changes to `@schovest/pi-todo` are documented here.
...
### Changed
- Refresh npm cover (`docs/cover.{svg,png}`) to share the unified card layout used across the `@schovest/pi-*` family.
...
### Added
- Localized TUI chrome (overlay heading, `/todos` section headers, status words) via `@schovest/pi-i18n`. 8 locales out of the box (de, en, es, fr, pt, pt-BR, ru, uk). LLM-facing strings (response envelope, reducer errors) stay English.

### Changed
- `@schovest/pi-i18n` is now a soft optional peer (`peerDependenciesMeta.optional: true`). The bridge and module-init `registerStrings` call use a dynamic `await import()` inside try/catch, so a standalone install of just `rpiv-todo` no longer fails with `Cannot find module '@schovest/pi-i18n'` — the extension stays online with English-only UI when the SDK isn't present, and lights up localization automatically when it is.
