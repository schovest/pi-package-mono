 ...
 * routes through one module. Backed by `@schovest/pi-i18n`'s SDK when
 * available; degrades to canonical-English fallbacks when not.
 *
 * - `t(key, fallback)` is `scope("@schovest/pi-todo")` if the SDK is
...
export const I18N_NAMESPACE = "@schovest/pi-todo";
